(() => {
  ...
  const unitsFromImportContext = () => {
    ...
    const map = new Map();
    const addUnit = (unit, sourceName = '') => {
      if (!unit) return;
      const rawName = unit.name || unit.unitName || unit.title || unit.label || unit.value || unit.id || '';
      ...
      if (!map.has(key)) {
        map.set(key, {
          id: String(unit.id || unit.unitId || key),
          name,
    ...
    if (ctx) {
      if (Array.isArray(ctx.units)) ctx.units.forEach(addUnit);
